...
def alpha_C4(G, c4e):
    """oracle-width for C4: orient copy-bearing edges toward lower C4-degree."""
    cdeg = {v: 0 for v in G}
    for e, c in c4e.items():
        u, v = tuple(e)
        cdeg[u] += c
        cdeg[v] += c
    ow = {v: 0 for v in G}
    for (u, v) in G.edges():
        if c4e[frozenset((u, v))] == 0:
            continue
...
def run_synthetic(sizes, seeds, families):
    ...
    print("\nFor C4, rho=2, so theory predicts speedup = Theta((m/alpha)^{rho-1})")
    print("= Theta(m/alpha). Compare with K3 where it was Theta(sqrt(m)/alpha).")
    return rows
