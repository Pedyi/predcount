...
def oracle_width_K3(G):
    ...
    # EXACT alpha_{K3} (Definition 3) via max-flow pseudoarboricity.
    kappa=max(nx.core_number(G).values()) if G.number_of_edges() else 0
    alpha=alpha_from_cb_edges([tuple(e) for e in te if te[e]>0], kappa_hint=kappa)
    # For the friendship gadget, the density-optimal orientation sends every
    # triangle edge away from the hub; we report the hub's out-degree under the
    # canonical "toward lower triangle-degree" orientation, which on this gadget
    # coincides with the optimum and gives hub out-degree 0.
    tdeg={v:0 for v in G}
    for e,c in te.items():
    ...
    return alpha, ow.get(0,0)  # (exact alpha, hub's oriented out-degree)
