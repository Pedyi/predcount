...
------------------------------------------------
  ...
  HEAVY-DROP : a heaviness-thresholding baseline in the SPIRIT of the
               variance-reduction idea of Luderssen, Neumann and Peng (2026).
               Edges whose predicted copy-count exceeds a threshold are removed
               from the sampling estimator (their contribution is handled
               separately), leaving the sampler a lighter residual.
               NOTE: this is NOT their deployed algorithm. Their practical
               method (EIS/NIS) is an induced-subgraph sampler, and their
               theoretical heaviness oracle is one they themselves dispense with
               in experiments (their datasets have small edge-heaviness). This
               baseline stands in only for the heaviness-thresholding MECHANISM;
               we do not reimplement EIS and do not claim to compare against it.
