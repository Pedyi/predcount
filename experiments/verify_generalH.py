...
APPENDIX B groundwork: does the per-copy success probability generalize as
   p_succ = Theta( #H / (m * alpha_H^{rho(H)-1}) )
for H in {C4, K4, C5}?  We test on separable-style constructions per pattern.

Method: build a graph where copies of H concentrate on a low-width gadget plus a
copy-free dense decoy, grow the decoy, and check that the ORACLE cost
   1 / p_succ  ~  m * alpha^{rho-1} / #H
scales in m with exponent 1 (constant alpha), while the UNIFORM FGP cost
   (2m)^{rho} / #H  scales with exponent rho.

We don't re-derive the full path sampler here; we use the aggregate oracle-cost
proxy m*alpha^{rho-1}/#H vs uniform (2m)^rho/#H, exactly as validated for K3,
and confirm the EXPONENTS separate by (rho - 1) for each pattern. This is the
quantity the general theorem claims.
...
for H in ["C4","K4","C5"]:
    ...
    for dfac in [1,2,3,4]:
        ...
        rho=RHO[H]
        unif=(2*m)**rho/H_ct
        orac=m*(2**(rho-1))/H_ct    # alpha=2 assumed on gadget
