...
Under perfect weighting (see Appendix B of the paper), the per-copy success
probability on an H-separable graph is

    p_succ = Theta( #H / ( m * alpha^{b_H - 1} ) ),

where b_H is the number of BASE-EDGE draws in the FGP decomposition (one factor
of alpha per base-edge normalizer that lacks a local pool; the closing draws are
Theta(1) after their local normalizers). Hence the SPACE bound is
Otil( m * alpha^{b_H} / #H ), and the IMPROVEMENT over the baseline (2m)^{-rho}
has m-exponent

    rho(H) - 1.

Two things worth stating precisely (both were sources of an earlier error):
  * The alpha-exponent is b_H, NOT rho(H)-1. For K3, b_{K3}=1, giving space
    Theta(m*alpha/#K3), matching Theorem 10. Writing alpha^{rho-1} was wrong
    (it gives alpha^{0.5} for K3, contradicting Theorem 10 / Lemma 9).
  * What THIS SCRIPT measures is the m-exponent of the improvement on SEPARABLE
    instances where alpha = Theta(1). There, alpha^{b_H} is a constant and the
    whole gain is m^{rho-1}, so the measured exponent is rho(H)-1 regardless of
    b_H. (The script therefore validates the m-exponent, not the alpha-power.)

THIS SCRIPT constructs separable instances (alpha = Theta(1)) and measures
...
def alpha_H(G, H):
    ...
    for (u, v) in G.edges():
        if H == "K3":
            ...
            dfs(u, v, L - 1, {u, v})
            bearing[frozenset((u, v))] = cnt
    # EXACT oracle-width = pseudoarboricity of the copy-bearing subgraph
    # (Definition 3), via max-flow. See src/alpha_exact.py.
    kappa = max(nx.core_number(G).values()) if G.number_of_edges() else 0
    cb_edges = [tuple(e) for e in bearing if bearing[e] > 0]
