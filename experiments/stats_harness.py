...
def graph_params(G):
    ...
    # EXACT alpha_{K3} = pseudoarboricity of triangle-bearing subgraph (Def 3).
    cb_edges = [tuple(e) for e in te if te[e] > 0]
    alpha = alpha_from_cb_edges(cb_edges, kappa_hint=kappa)
...
def run(families, sizes, n_graphs, n_runs, k_budget):
    ...
    print("=" * 100)
    print(f"{'family':>9}{'speedup ~ m^b':>22}{'alpha ~ m^a':>20}"
          f"{'predicted b':>14}")
    for fam in families:
        ...
        pred_b = 0.5 * (1.0 - e_a)   # theory: b = (rho(K3)-1)(1-a) = 0.5(1-a)
        print(f"{fam:>9}  {e_s:>6.2f} [{cs[0]:>5.2f},{cs[1]:>5.2f}]"
              f"  {e_a:>6.2f} [{ca[0]:>5.2f},{ca[1]:>5.2f}]"
              f"{pred_b:>14.2f}")
    print("\nTheory: speedup exponent b = (rho(K3)-1)(1-a) = 0.5(1-a), where a is")
    print("the alpha growth exponent. On constant-alpha (separable) families a~0")
    print("so b~0.5; when alpha grows, b falls. (The two exponents do NOT simply")
    print("sum to 0.5 unless a~0.)")
    return rows
