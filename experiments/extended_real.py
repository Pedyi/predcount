...
# name -> (url, size class)
#
# NOTE ON WHICH DATASETS ARE REPORTED IN THE PAPER (Table 2):
#   The paper reports the ELEVEN datasets from "ca-GrQc" through "amazon0302"
#   below. The last two entries here, "web-Stanford" (large web graph) and
#   "bio-CE-GN" (biological network), are DEFINED and RUNNABLE by this script but
#   are NOT reported in the paper's table. The reason is purely computational:
#   the exact success-probability machinery and the C4 codegree proxy are very
#   slow on web-Stanford (~2M edges) and we did not complete those runs in time;
#   bio-CE-GN was the sole "biological" representative and we dropped the
#   "biological" domain claim rather than report a single-graph domain. Neither
#   omission affects the finding: on every dataset we DID measure,
#   alpha_H/kappa stayed a constant fraction of 1. To reproduce exactly the
#   eleven-graph table, run with --datasets set to the first eleven names (or
#   simply let the two large/slow entries be skipped). We keep the two extra
#   entries here so the study can be extended by anyone with more compute.
DATASETS = {
    # ---- the ELEVEN datasets reported in the paper (Table 2) ----
    # collaboration
    ...
    # ---- extra datasets NOT reported in the paper (see note above) ----
...
def k3_stats(G):
    ...
    T = sum(te.values()) // 3
    # EXACT oracle-width alpha_{K3} = pseudoarboricity of the triangle-bearing
    # subgraph (Definition 3), via max-flow. See src/alpha_exact.py.
    kappa_hint = max(nx.core_number(G).values()) if G.number_of_edges() else 0
    cb_edges = [tuple(e) for e in te if te[e] > 0]
    alpha = alpha_from_cb_edges(cb_edges, kappa_hint=kappa_hint)
...
def k4_stats_sampled(G, deg, adj, te, n_sample=20000, rng=None):
    ...
    # EXACT oracle-width (pseudoarboricity) on the sampled K4-bearing structure.
    # NOTE: since K4 copies are sampled (exact K4 counting is infeasible at
    # scale), the K4-bearing edge set here is an approximation, so alpha4 is an
    # estimate; but it is computed by the correct max-flow routine, not a greedy
    # orientation. See src/alpha_exact.py.
    k4_cb = [tuple(e) for e in wk]
    kappa_hint = max(nx.core_number(G).values()) if G.number_of_edges() else 0
    alpha4 = alpha_from_cb_edges(k4_cb, kappa_hint=kappa_hint)
